...
async function refreshModels(silent) {
  ...
  try {
    const out = await SH_CHAT.fetchModels(prov);
...
function renderSugs() {
  ...
  if (!bar) return;
  bar.innerHTML = '';
  if (SH_CHAT.count() > 1) { bar.style.display = 'none'; return; }
  bar.style.display = '';
...
function openSession(id) {
  ...
  ((s && s.msgs) || []).forEach((m) => SH_CHAT.add(m.content, m.role === 'user' ? 'user' : 'bot', (m.via || '').replace(/^via /, '')));
...
async function onFiles(files) {
  ...
  for (const f of arr) {
    const parsed = await SH_CHAT.readFile(f);
...
async function send() {
  ...
  if (!canSend(provider, model)) {
    if (q) SH_CHAT.add(q, 'user');
    SH_CHAT.add(provider === 'ollama' ? ollamaHint() : 'No models: set the key in Vercel env first, then redeploy.', 'bot', 'error');
    return;
  }
  if (provider === 'ollama' && !SH_CHAT.ollamaUrl()) {
    try {
      const all = await SH_CHAT.fetchAllModels();
      if (!(all.ollama && all.ollama.configured)) {
        if (q) SH_CHAT.add(q, 'user');
        SH_CHAT.add(ollamaHint(), 'bot', 'error');
        return;
      }
    } catch (e) {}
  }
  ...
  if (files.length) SH_CHAT.addFiles(files, 'user');
  if (q) SH_CHAT.add(q, 'user');
  else if (files.length) SH_CHAT.add('(files attached: ' + files.map((f) => f.name).join(', ') + ')', 'user');
  clearPending();
  ...
  updateStatusSub();
  const msgs = SH_CHAT.history(10).concat([{ role: 'user', content: q || ('Analyze attached files: ' + files.map((f) => f.name).join(', ')) }]);
  const botNode = SH_CHAT.add('…', 'bot');
  const sysMsgs = [SH_CHAT.sysMsg()].concat(msgs);
  const intent = SH_CHAT.intentFor(q || (files.length ? files[0].name : ''));
  ...
  const onDelta = (d) => {
    ...
    SH_CHAT.updateBot(botNode, botNode._partial = (botNode._partial || '') + d, '');
    SH_CHAT.scrollBottom(true);
  };
  try {
    let ans;
    const useLocal = provider === 'ollama' && SH_CHAT.ollamaUrl();
    if (useLocal) {
      try { ans = await SH_CHAT.viaProxyStream(provider, model, sysMsgs, files, onDelta); }
      catch (pe) { ans = await SH_CHAT.ollamaDirectStream(model, msgs, files, onDelta); }
    } else {
      try { ans = await SH_CHAT.viaProxyStream(provider, model, sysMsgs, files, onDelta); }
      catch (pe) {
        try { ans = await SH_CHAT.viaProxy(provider, model, sysMsgs, files); }
        catch (pe2) { throw pe2; }
      }
    }
    if (ty) ty.classList.remove('show');
    SH_CHAT.updateBot(botNode, ans.reply, ans.via);
    if (store().get('sh.tts', 'off') === 'on') SH_CHAT.speak(ans.reply);
  } catch (e) {
    if (ty) ty.classList.remove('show');
    SH_CHAT.updateBot(botNode, errText(e), 'error');
  }
...
export function initChatUI() {
  ...
  if (oinp) {
    oinp.value = SH_CHAT.ollamaUrl();
    oinp.placeholder = 'Ollama URL, e.g. http://localhost:11434';
    let dt = 0;
    oinp.addEventListener('input', () => { SH_CHAT.setOllamaUrl(oinp.value); clearTimeout(dt); dt = setTimeout(() => refreshModels(false), 700); });
  ...
  if (tts) {
    ...
    tts.addEventListener('change', () => { store().set('sh.tts', tts.checked ? 'on' : 'off'); if (!tts.checked) SH_CHAT.stopSpeak(); });
  }
  ...
  ta.addEventListener('focus', () => { setTimeout(() => SH_CHAT.scrollBottom(), 120); });
  ...
  document.addEventListener('keydown', (e) => { if (e.key === 'Escape') { closeSide(); stopRec(); SH_CHAT.stopSpeak(); } });
...
if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', initChatUI);
else initChatUI();